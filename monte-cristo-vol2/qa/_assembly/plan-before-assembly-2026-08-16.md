...
1. **The builder may not redesign a page.** The production lead owns redesign.
   **Owner instruction, 2026-08-16:** after a genuine v4 ceiling, the production
   lead autonomously rewrites composition and scene blocking around the same
   story and exact dialogue, then continues without asking. Owner input is
   required only if story facts, exact strings, reference locks, billing path or
   page count must change.
...
- **Exactly one unmistakable dominant panel, targeting 45–70% of the page.**
  Declared in the prompt. The model will not infer hierarchy from a list of
  equal-sounding panels. **Owner precedent, 2026-08-15:** a de minimis
  one-to-three-point rendered variance is nonblocking when hierarchy remains
  unmistakable and no reader or story harm results; critics do not measure to
  manufacture a percentage finding.
...
undrunk.
**Dominant:** the general's staircase, two men climbing it side by side — 48%.
**Locations:** 1. **Panels:** 5.
**Output:** `qa/production/page-08/candidates/page-08-v1.png`
...
> grayscale, partial hair, and travel clothes.
>
> **Page staging rule, binding on every panel:** **Fernand holds the left, the
> Count holds the right**, and every balloon sits on its owner's side — including
> in panel 4, where Fernand is off-panel and his balloon still enters from the
> left.
>
> ### Panel 1 — **DOMINANT, roughly 48% of the page**, the whole upper portion
>
> **Two men climbing the general's staircase side by side**, seen from below and
> slightly behind so the staircase towers — the same wide gilt staircase as the
> supplied setting plate and as page 6. **Fernand at the left, one step ahead and
...
> black, taller even a step down, unhurried, entirely composed.** Behind and below
> them, the party as warm blurred light — **no individuated faces.**
>
...
>
> ### Panel 2 — roughly 8%, a narrow band below
>
> Fernand alone, close, **half-turned back over his shoulder — flattered, and not
> sure why.** The moustache and the receding temple both clearly legible at this
> scale.
>
> One warm-ivory balloon, left, tail to his mouth, exactly:
>
> `Four times. Why?`
>
> ### Panel 3 — roughly 18%, a wide band below that
>
> **The Count's hand on the banister rail — the poised hand**, long, clean, still,
> hard-edged, occupying the right of the band; the gilt rail and a sliver of
> Fernand's decorated chest at the left. Faces may be cropped away entirely; the
> hand is the subject.
>
> Two warm-ivory balloons. The Count's runs **wide and flat** across the upper
> band, right-anchored, tail running down to his hand's owner off the top of the
> frame; Fernand's short reply sits at the **lower left**. Exact strings, in this
> reading order:
>
...
> `I earned it.`
>
> ### Panel 4 — roughly 8%, a narrow band below that
>
> **Tight on the Count.** Head and shoulders, straight on, evenly lit. **Nothing
> on his face at all** — not amusement, not menace, not sympathy; a face with the
> expression removed. Fernand is off-panel to the left.
>
> Two warm-ivory balloons, both small. The Count's sits at the **right**, tail to
> his mouth; Fernand's sits at the **left**, tail running **off the left edge of
> the panel** toward the unseen speaker — it must not point at the Count, at empty
> room, or at any object. Exact strings, in this reading order:
>
> `In Spain?`
>
> `In Greece.`
>
> ### Panel 5 — roughly 18%, a wide horizontal band across the bottom
>
> **The volume's primary motif begins here and must be unmistakable.** At the
> left, **a raised glass of red wine held up in a thick ringed hand** — Fernand's
> toast, going on above and outside the frame, unnoticed. At the right, larger and
> nearer, **the Count's long pale hand setting an identical glass down on a marble
> top — and that glass is FULL to the same level as the raised one.** It is being
> set down, not lifted; not sipped, not tasted, not touched to the lip. Both
> glasses are the same glass from the supplied object board. Faces are cropped
> out of this band entirely.
>
> Two warm-ivory balloons, both the Count's, at the right of the band, the second
> below the first, tails running off the top edge toward the unseen speaker above.
> Exact strings, in this reading order:
>
> `Of course. Greece.`
>
...
> px** wide, except the two long strings in panels 3 and 5, which run **wide and
...
> military portrait rendered legibly; no Albert; no woman in burgundy-black.
> Standing prohibitions: no identity collision, no duplicated person, hand or
> object — **the two glasses in panel 5 are two different glasses held by two
> different people and must be staged so that no reader reads them as a duplicated
> object** — no fused fingers, no illegible text, no crop sheet, no outer
> decorative frame.
>
> ## Reference images
...
> all-caps.** **The Count owns seven balloons and Haydée owns six**; every balloon
...
| Role | May | May never |
|---|---|---|
| **Builder** | generate one candidate, self-audit it, derive proofs, submit to the critic, prepare the next page's prompt | approve, promote, write to `pages/`, reroll on taste, generate page N+1 before N is promoted |
| **Critic** | read, transcribe, judge, return a verdict and mandatory findings | edit, regenerate, promote, propose prompt wording |
| **Production lead** | release a page, promote bytes, hold a batch, redesign a page that hit the v4 ceiling | generate, or approve their own generation |
...
inputs** → self-audit → derive the 600 × 900 and 768 × 1152 proofs → hand to the
...
> **Per-page appendix:** `12-PRODUCTION-PLAN.md` §6, page `[N]`
>
...
>    no unmistakable single dominant panel in the intended **approximately
>    45–70%** band; more than **two** locations; more than one dominant turn.
>    Judge visible reading hierarchy, not a ruler.
...
>   void;
> - exact panel percentages beyond the single dominant-panel rule. **Owner
>   precedent, 2026-08-15:** a de minimis one-to-three-point variance is
>   nonblocking when the page still has one unmistakable dominant panel and the
>   variance causes no reader or story harm. Do not measure merely to manufacture
>   a percentage finding;
...
>
> Open `12-PRODUCTION-PLAN.md` §6 and find this page's appendix. It names this
...
>
> **Page:** `[N]`
> **Prompt:** `12-PRODUCTION-PLAN.md` §5, page `[N]` — use it **verbatim**.
...
> 6. Submit to the independent critic. **Do not reroll on your own judgment.** A
>    candidate you dislike for cosmetic reasons still goes to the critic — you do
>    not get to spend generations on taste.
>
...
> 4. Every page has one dominant panel at 45–70%.
...
- **v4 ceiling.** If v4 of a page still returns REVISE, the composition has
  failed. Stop patching the prompt: redesign the page geometry or split the page
  and tell the production lead. Do not generate a v5 against the same design.
...
**Owner architecture override, 2026-08-15 — Page 1 only.** The rendered upper
panel's approximately 73% share against the 70% construction target is approved
and nonblocking. The page has one unmistakable dominant panel, the three-roof
band remains fully legible, and the owner judged the composition successful.
Do not measure or reject this Page 1 candidate on that three-point variance.
...
- Two glasses are in play on this page. They must read as two separate objects in two separate places, never as one duplicated object — different surfaces, different light, different framing.
...
- `In Greece.` is spoken by Fernand from off-panel left; the tail exits the panel edge on his side.
- The Count wears no decoration on this page. Fernand's decorations are the only ones in frame, and their contrast with the Count's unrelieved black is the point of the staging.
- The 8% panels carry short strings only; check that neither exceeds 20 words and that both transcribe exactly and comfortably from the 600 × 900 proof.
...
The production lead either redesigns the page geometry or splits the page in
two — and if it splits, `07-PAGE-CONTRACT.md` and `08-FULL-SCRIPT.md` are updated
in the same action, because they are the authority and must not drift behind the
art. §5's prompt for the affected page is rewritten to match before the next
generation.

The executor may **not** redesign a page on their own initiative. A page that
cannot be built as specified stops and goes back to the plan owner.
...
| **v4 ceiling** | any page whose v4 returns REVISE | production lead redesigns the geometry or splits the page |
...
> approves its own work. Do not redesign any page; if a page cannot be built as
> specified, stop and report it. Do not generate prototypes — every generated
...
- [x] The executor is forbidden from redesigning pages — §1, §7, §10.
