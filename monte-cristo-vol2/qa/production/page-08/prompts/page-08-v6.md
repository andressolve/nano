...
**Redesigned page staging rule, binding on every panel:** **Fernand holds the
left and the Count holds the right.** The question and answer are isolated into
separate one-speaker panels: panel 4 shows only the Count with only his
question; panel 5 shows only Fernand with only his answer. Never put `In
Spain?` and `In Greece.` in the same panel. The final glass action is enlarged
and physically separates raised from resting by a full glass-height of empty
space.
...
### Panel 2 — roughly 9%, a narrow band below
...
### Panel 3 — roughly 13%, a wide band below that
...
### Panel 4 — roughly 7%, a narrow one-speaker band

**The Count alone**, head and shoulders at the right, straight on and evenly
lit. Nothing on his face — not amusement, menace or sympathy. Fernand is not
visible and owns no balloon in this panel.

Exactly one small warm-ivory balloon at the right, tail visibly terminating at
the Count's mouth, exactly:

`In Spain?`

### Panel 5 — roughly 7%, a separate one-speaker band

**Fernand alone**, head and shoulders at the left, moustache and receding
...
Exactly one small warm-ivory balloon at the left, tail visibly terminating at
Fernand's mouth, exactly:

`In Greece.`

### Panel 6 — roughly 16%, a wide horizontal action band across the bottom

**The volume's primary motif begins here and must be physically
unmistakable.** Divide the action by height and surface, not with a drawn
divider. At the left, Fernand's thick ringed hand holds a full glass **at chest
height**, its entire foot and stem suspended against dark cloth with **at least
one full glass-height of empty black space beneath it**. There is no marble,
table edge or supporting surface anywhere under Fernand's glass. At the right,
the marble top is visible and the Count's long pale hand is **pressing the foot
of a separate identical full glass onto that marble**. One glass is aloft over
empty space; the other visibly touches stone. Neither is near a mouth.

Show enough of the Count's lower face and black shoulder at the far right to
anchor both of his balloons. Fernand is silent here. Two warm-ivory balloons,
both the Count's, stacked at the right with tails visibly terminating at the
Count, never at Fernand or either glass. Exact strings, in this reading order:
...
px** wide, except the two long strings in panels 3 and 6, which run **wide and
...
object — **the two glasses in panel 6 are two different glasses in two different
...
V6 MANDATORY CORRECTION ONLY: show the Count's face and mouth clearly at upper RIGHT; render TWO fully separate warm-ivory Count balloons at the far RIGHT with visible space between them—upper balloon exactly `Of course. Greece.` with its own short tail terminating at the Count's mouth; lower balloon exactly `Forgive me. I am a stranger here and I get your wars wrong.` with its own separate short tail also terminating at the Count's mouth. No joined balloon outline, no shared tail, no tail leftward, no tail to either glass/hand/empty space. Fernand silent.
