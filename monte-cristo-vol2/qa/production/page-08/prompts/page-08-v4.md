...
V4 FINAL MANDATORY CORRECTION ONLY: Correct ONLY Panel 4 ownership while freezing every passed v3 element: preserve dominant ~55% Panel 1, compressed lower bands, no wall/background military portraits/decorated figures/orders/ribbons, decorations only on live Fernand, raised Fernand glass vs Count setting down distinct full glass, every exact string and identity/register/continuity. Panel 4 map is absolute: visible Count on RIGHT owns `In Spain?`, balloon on RIGHT with tail visibly terminating at Count's mouth/head; off-panel Fernand on LEFT owns `In Greece.`, balloon on LEFT with tail exiting LEFT edge into empty off-panel space, never crossing toward Count.
