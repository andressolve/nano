...
V4 FINAL MANDATORY CORRECTION ONLY: prior versions preserved all content but rendered nominal prose sizing as black glyphs only 19–23 px tall. Render genuinely oversized literary prose: choose at least 72 px nominal source type—or whatever larger nominal size is required—so every lowercase black x-height is 40–44 actual source pixels and at least 24–26 pixels on the 600×900 proof. Use two fixed parchment fields near the maximum allowed width (about 88% / 900 source pixels), with at least 42 px padding. Reserve enough uninterrupted blank wall above the window for the entire first field and enough dark sky for the second; enlarge the existing fields rather than shrinking text. Reframe the room/window lower inside the same dominant panel only as necessary to protect the larger upper field, while keeping the Count small and the window/room legible. Preserve exactly two prose fields, all four exact strings without abbreviation, the 65/35 two-panel structure, roof cues, identity, set, motif, palette and every other instruction. No condensed font, tight scaling, nominal-size substitution, text overlay repair or post-generation lettering.
