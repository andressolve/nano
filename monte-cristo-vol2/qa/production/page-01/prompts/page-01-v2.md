...
V2 MANDATORY CORRECTION ONLY: both prose fields failed the 40 px source-lettering floor. Render every visible prose letterform at a true 40–42 px height on the 1024×1536 source—not a nominal font size and not merely a 40 px line box—with generous line pitch. Enlarge the parchment fields within their already specified dark-wall and dark-sky lanes as needed; preserve every exact string, the two-panel 65/35 architecture, all roof/set/identity/motif commitments, and every other instruction unchanged. Never shrink or condense the type.
