...
**V4 FINAL MANDATORY CORRECTION ONLY:**

In Panel 2, the third balloon `On the strength of a letter written in a language none of you reads.` must have exactly one clean tail visibly terminating at Fernand's visible mouth or face on the LEFT. The tail must not exit the panel, point to empty space, point to the rail, point to a seated peer, point to any other figure, or read as an off-panel voice. Preserve Fernand's unmistakable ownership of all three Panel 2 balloons and exactly six balloons across the page.

**PRESERVE EVERY CRITIC-V3-PASSED ELEMENT:** preserve Panel 3 exactly as cleared—exactly two Fernand balloons and exactly two off-panel tails total, one clean tail per balloon, both visibly exiting the lower Panel 3 boundary toward Fernand on the Chamber floor, neither terminating on a seated peer, and no duplicate or orphan tail. Preserve all seven exact strings and their order; Panel 1 is silent; the Count appears only in the gallery with a hungry face and a normal-sized `Go on. Win it.`; Fernand is visibly winning, fully decorated, warm, upright and in command; one peer is half-rising; Danglars is silent and untroubled with full side whiskers and no moustache; no Haydée or any woman appears; no case, document, open door, torch, candle or warm light appears; and the same cold-daylight Chamber continuity, identities, anatomy, five-panel architecture, dominant turn, Velvet Cinema register and exact 1024 × 1536 RGB canvas remain unchanged. Correct nothing else and do not redesign the page.
