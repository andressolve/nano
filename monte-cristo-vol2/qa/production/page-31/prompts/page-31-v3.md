...
**V3 MANDATORY CORRECTION ONLY:**

Panel 3 must contain exactly TWO Fernand balloons and exactly TWO tails total: one clean, single tail from each balloon. Each one tail must visibly cross and exit the Panel 3 boundary toward off-panel Fernand on the Chamber floor and must never terminate in or point at any seated peer, Danglars, or gallery figure. There must be no duplicate third tail, no second tail sprouting from either balloon, no orphan tail fragment, and no separate white outline or extension that reads as another tail. Every visible peer remains silent.

**PRESERVE EVERY CRITIC-V2-PASSED ELEMENT:** preserve all seven exact strings and their order; Fernand owns exactly six balloons and the Count exactly one; Panel 1 is silent; Panel 3 reads unmistakably as Fernand continuing from the off-panel floor; the Count appears only in the gallery with a hungry face and a normal-sized `Go on. Win it.`; Fernand is visibly winning, fully decorated, warm, upright and in command; one peer is half-rising; Danglars is silent and untroubled with full side whiskers and no moustache; no Haydée or any woman appears; no case, document, open door, torch, candle or warm light appears; and the same cold-daylight Chamber continuity, identities, anatomy, five-panel architecture, dominant turn, Velvet Cinema register and exact 1024 × 1536 RGB canvas remain unchanged. Correct nothing else and do not redesign the page.
