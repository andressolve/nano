...
V2 MANDATORY CORRECTION ONLY: Render exactly THIRTEEN balloons total. The string `You may not like what I bring back.` appears exactly ONCE on the whole page, owned only by Beauchamp on the RIGHT in Panel 4, with its tail to Beauchamp; there is no duplicate, no Albert-side copy, and no Albert-side tail for that string. Preserve every critic-passed v1 essential unchanged: exactly four panels; all other exact strings, order, and ownership; Albert-left/Beauchamp-right identity separation; the same newspaper with no legible print; the poor press room; Panel 3's genuine offer with Beauchamp's spectacles in hand; dominant Panel 4 refusal with Albert standing and both hands flat; anatomy; Velvet Cinema register; and the exact 1024 × 1536 RGB portrait canvas.
