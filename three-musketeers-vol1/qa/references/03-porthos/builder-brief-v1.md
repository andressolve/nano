Builder brief — 03-porthos — candidate v1 — three-musketeers-vol1
...
View (b): full length, hat on, chest out, fists on hips, feet planted wide,
the gold front of the baldric fully visible.

View (c): a true ninety-degree profile from plumes to boots, same light,
keeping the bulk, the barrel chest, the moustache points, and the plumes.
...
SAVE. Write the returned PNG byte for byte to books/three-musketeers-vol1/refs/candidates/03-porthos-v1.png. Write the prompt text above, exactly as issued, to books/three-musketeers-vol1/refs/candidates/03-porthos-v1.md.

AUDIT (a report, not a verdict). Write books/three-musketeers-vol1/refs/candidates/03-porthos-v1.audit.md with: the file's pixel size as a script reports it (must be 1536 × 1024); whether all four views are present (three-quarter head, full length, strict profile, the named state) — for a plate, whether any figure appears; for a board, whether both attached characters appear and read as the same faces as their sheets; whether any lettering, label, watermark, or border appears; anything else you noticed. Then stop.
