Builder brief — 07-rochefort — candidate v1 — three-musketeers-vol1
...
on, from the left side so the scar is fully visible, large enough to lock the
square face, heavy black brows, thin moustache with no beard, and the scar
...
View (c): a true ninety-degree left profile from hat to boots, same light,
the scar running across the cheek, the thin moustache, no beard.

Views (a)–(c) have no scenery.

View (d): the same man hatless and soaked through, hair flat, standing on
a stone step with his right arm held against his chest by his left hand, the
other fist closed, the scar and the thin moustache unchanged.
...
SAVE. Write the returned PNG byte for byte to books/three-musketeers-vol1/refs/candidates/07-rochefort-v1.png. Write the prompt text above, exactly as issued, to books/three-musketeers-vol1/refs/candidates/07-rochefort-v1.md.

AUDIT (a report, not a verdict). Write books/three-musketeers-vol1/refs/candidates/07-rochefort-v1.audit.md with: the file's pixel size as a script reports it (must be 1536 × 1024); whether all four views are present (three-quarter head, full length, strict profile, the named state) — for a plate, whether any figure appears; for a board, whether both attached characters appear and read as the same faces as their sheets; whether any lettering, label, watermark, or border appears; anything else you noticed. Then stop.
