Builder brief — 01-dartagnan — candidate v1 — three-musketeers-vol1

You are a reference-sheet builder. You generate exactly one candidate from the prompt below, write it to disk, write a non-gating audit, and stop. You never judge whether the sheet is good, never regenerate on your own opinion, never promote, and never open any other file in this book.

GENERATION. Codex in-app image generation on the ChatGPT subscription only. No API key, no image CLI, no separately billed path. One call. Size 1536 × 1024.
IMAGE INPUTS: none. Text prompt only.

PROMPT — issue this text exactly, nothing added, nothing removed:

----- BEGIN PROMPT -----
A character reference sheet, 1536 × 1024 landscape, four views of the same
figure on one plain warm-paper ground, no lettering of any kind, no labels, no
borders between views.

d'Artagnan, eighteen, from Gascony. A boy, not a man: long narrow face, sharp
chin, a long straight nose, dark brows, dark brown eyes set close and quick,
sun-brown skin. No beard and no moustache of any kind; the only clean-shaven
male face in the book. Black hair, thick and unruly, falling into his eyes,
cut roughly to the jaw. Lean, all elbows and knees, narrow shoulders. He never
stands still: weight forward on the balls of his feet, one hand on the hilt.
Habitual gesture: pushing his hair out of his eyes with the back of his sword
hand. Expression at rest: about to say something he should not.

Default costume, views (a), (b), (c): a doublet of faded, patched ochre-yellow
wool over a plain shirt, brown breeches, worn brown riding boots, a grey felt
hat with one broken feather, and his father's sword at his hip: an old long
rapier with a plain swept steel hilt, visibly too long for him. No cloak.

View (a): a true three-quarter head-and-shoulders study in even light, hat
on, large enough to lock the long narrow face, sharp chin, long straight nose,
close-set quick dark eyes, low dark brows, and the black hair falling into his
eyes. Clean-shaven, unmistakably.

View (b): full length, hat on, weight forward on the balls of his feet, one
hand on the hilt, the sword's length against his leg showing it is too long
for him, the other hand pushing his hair back.

View (c): a true ninety-degree profile from hat to boots, same light, keeping
the long nose, sharp chin, narrow shoulders, and the sword's length.

Views (a)–(c) have no scenery and no props beyond the sword and hat, leaving
only the plain warm paper.

View (d): the same boy at the end of a two-day ride: the same clothes with
mud to the waist, hat in his hand, hair wet and in his eyes, standing very
straight in a doorway of light.

d'Artagnan must never be confused with Aramis. Differentiators that must
survive grayscale and thumbnail: no moustache against Aramis's small neat
one; black rough hair against blond curls; ochre with no cloak against the
blue cassock; weight forward against weight on one hip. He must never be
confused with Rochefort: a beardless boy's face against a scarred man's; ochre
against black; a broken grey feather against a red one.

Lantern and Steel. Brush-and-ink line on warm laid paper with opaque, matte gouache washes: lamp-black line, raw umber and yellow ochre, Prussian blue for the Musketeers, vermilion for the Cardinal, lead white for lace and collars, a little gold. Seventeenth-century France lit like a lantern film: candle and lantern light indoors with real cast shadows and real depth, rain-grey and river-grey outdoors, dust and dawn on the road. Visible brush line, visible paper grain, gouache drying chalky at the edge of a wash; wool, leather, wet stone, steel, and wax. Not smooth prestige-oil realism. No glossy concept-art surfaces, no airbrushed skin, no engraved cross-hatching, no anime proportions, no children's-book softness, no plastic 3D render.

Their faces must remain structurally distinct even in profile, reduced scale,
grayscale, partial hair, and travel clothes.
----- END PROMPT -----

SAVE. Write the returned PNG byte for byte to books/three-musketeers-vol1/refs/candidates/01-dartagnan-v1.png. Write the prompt text above, exactly as issued, to books/three-musketeers-vol1/refs/candidates/01-dartagnan-v1.md.

AUDIT (a report, not a verdict). Write books/three-musketeers-vol1/refs/candidates/01-dartagnan-v1.audit.md with: the file's pixel size as a script reports it (must be 1536 × 1024); whether all four views are present (three-quarter head, full length, strict profile, the named state) — for a plate, whether any figure appears; for a board, whether both attached characters appear and read as the same faces as their sheets; whether any lettering, label, watermark, or border appears; anything else you noticed. Then stop.

SUBMIT RULE. The candidate is submitted as it is, even if your audit says it failed. The only regeneration allowed without a critic verdict is for a failed generation — wrong dimensions, a corrupt or truncated file, an empty or refused result — and in that case delete the bad file and regenerate the same version once. Never regenerate because you dislike the drawing.

Reply with: the output path, its SHA-256, and the audit path. Nothing else.
