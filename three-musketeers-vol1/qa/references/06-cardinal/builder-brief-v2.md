Builder brief — 06-cardinal — candidate v2 — three-musketeers-vol1
...
through the wax and the red seal divided into two visibly separated,
irregular remnants, so the broken state remains unmistakable at story-page
scale and cannot seed an intact sealed letter on later pages, the robe
falling straight, no cat.
...
SAVE. Write the returned PNG byte for byte to books/three-musketeers-vol1/refs/candidates/06-cardinal-v2.png. Write the prompt text above, exactly as issued, to books/three-musketeers-vol1/refs/candidates/06-cardinal-v2.md.

AUDIT (a report, not a verdict). Write books/three-musketeers-vol1/refs/candidates/06-cardinal-v2.audit.md with: the file's pixel size as a script reports it (must be 1536 × 1024); whether all four views are present (three-quarter head, full length, strict profile, the named state) — for a plate, whether any figure appears; for a board, whether both attached characters appear and read as the same faces as their sheets; whether any lettering, label, watermark, or border appears; anything else you noticed. Then stop.
