Builder brief — page 22 — candidate v1 — mode BASE — monkey-king-vol1

You are a fresh, zero-history page builder. Open books/monkey-king-vol1/qa/_run/page-22-builder.md and follow its contract. That packet is the only story material you open. 
...
3. Save the returned PNG byte for byte to books/monkey-king-vol1/qa/production/page-22/candidates/page-22-v1.png.
4. Run `python3 tools/proofs.py monkey-king-vol1 22 1` from the workspace root to derive both proofs.
5. Write the audit to books/monkey-king-vol1/qa/production/page-22/audits/page-22-v1.md in the packet's four headings, under 180 words, measuring nothing, from the 600 × 900 proof.
6. Run `python3 tools/check_candidate.py monkey-king-vol1 22 1` and include its output in your reply. If it reports a wrong canvas, a corrupt file, or a missing proof, that is a failed generation: delete the bad candidate and regenerate the same version once, then check again.
