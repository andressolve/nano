Builder brief — 01-wukong — candidate v2 — monkey-king-vol1
...
SAVE. Write the returned PNG byte for byte to books/monkey-king-vol1/refs/candidates/01-wukong-v2.png. Write the prompt text above, exactly as issued, to books/monkey-king-vol1/refs/candidates/01-wukong-v2.md.

AUDIT (a report, not a verdict). Write books/monkey-king-vol1/refs/candidates/01-wukong-v2.audit.md with: the file's pixel size as a script reports it (must be 1536 × 1024); whether all four views are present (three-quarter head, full length, strict profile, the named state) — for a plate, whether any figure appears; for a board, whether both attached characters appear and read as the same faces as their sheets; whether any lettering, label, watermark, or border appears; anything else you noticed. Then stop.
